...
class RolloutWorker:
    ...
    def generate_episode(self, episode_num=None, evaluate=False,epsilon=0):
        ...
        while not terminated and step < self.episode_limit:
            ...
            for agent_id in range(self.n_agents):
                ...
                if self.args.alg == 'maven':
                    ...
                    action,action_logp = self.agents.choose_action(obs[agent_id], last_action[agent_id], agent_id,
                                                       avail_action, epsilon,None, evaluate)
                # generate onehot vector of th action
                action_onehot = np.zeros(self.args.n_actions)
                action_onehot[action] = 1
                actions.append(action)
                actions_logp.append(action_logp)
            ...
            u_logp.append(np.reshape(actions_logp, [self.n_agents, 1]))  # array (3,1)
        ...
        for i in range(step, self.episode_limit):
            ...
            u_logp.append(np.zeros([self.n_agents, 1]))
