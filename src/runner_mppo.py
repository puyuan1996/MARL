import numpy as np
import os
from common.rollout_ppo import RolloutWorker, CommRolloutWorker
# from agent.agent import Agents, CommAgents
from common.replay_buffer import ReplayBufferPPO as ReplayBuffer
import matplotlib.pyplot as plt

from ac_discrete.qmix_ppo import QMIX_PG  # TODO
from agent.agent_pg_softmax_ppo import Agents
...
class Runner:
    def __init__(self, env, args):
        ...
        if args.learn and args.alg.find('coma') == -1 and args.alg.find('central_v') == -1 and args.alg.find('reinforce') == -1:  # these 3 algorithms are on-poliy
            self.actor_critic_buffer = ReplayBuffer(args, args.ppo_buffer_size)
        ...
        self.save_path = self.args.result_dir + '/linear_mix/'+ 'qmix_ppo' + '/' + tmp + '/'+ args.map#_gradclip0.5
        ...
        for epoch in range(self.args.n_epoch):
            ...
            if self.args.alg.find('coma') > -1 or self.args.alg.find('central_v') > -1 or self.args.alg.find(
                ...
                #         min(self.actor_critic_buffer.current_size, self.args.actor_batch_size))  # 16 episodes  # 16
                    self.qmix_pg_learner.train_actor(mini_batch, self.args.episode_limit,epoch)
        self.plt(num)
