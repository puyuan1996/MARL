...
class QMIX_PG():
    def __init__(self,agent, args):
        ...
        self.model_dir = args.model_dir + '/' + 'ac_local—total_counterfactual'+ '/' + f'{args.actor_buffer_size}' + '/' + args.alg + '/' + args.map  # TODO
        # 如果存在模型则加载模型
    ...
    def train_actor(self, batch, max_episode_len):#EpisodeBatch
        ...
        # Calculated baseline
        q_taken = torch.gather(q_vals, dim=3, index=actions).squeeze(3) #(1,60,3) # todo local Q
        # q_taken = self.eval_qmix_net(q_taken, s).detach()  # TODO # (1,60,1)
        ...
        q_targets_mean = torch.sum(actions_prob * q_vals, dim=-1).detach()  # (1,60,3)
        q_targets_counterfactual=[]
        ...
        q_taken_detached = q_taken.detach()
        for i in range(self.n_agents):
            q_taken_temp = copy.deepcopy(q_taken_detached)
            q_taken_temp[:, :, i] = q_targets_mean[:, :, i]
            q_targets_counterfactual.append(q_taken_temp)
        q_targets_counterfactual = torch.stack(q_targets_counterfactual,dim=2) #[1, 60, 3, 3]
        q_targets_counterfactual = q_targets_counterfactual.view(episode_num,-1,self.n_agents)  #[1, 60*3, 3]
        s_repeat = s.repeat(repeats=(1, self.n_agents,1)) #  (1,60,3)-> # (1,60*3,3)

        q_total_target = self.eval_qmix_net(q_targets_counterfactual, s_repeat).detach() # TODO #  (1,60*3,1)
        # q_total_target = q_total_target.repeat(repeats=(1, 1, self.n_agents)) # (1,60,3)
        baseline = q_total_target.view(episode_num,-1,self.n_agents) # (1,60,3)
        ...
        advantages = (advantages_original - advantages_original.mean(dim=0)) / advantages_original.std(dim=0) #一个batch 32episodes,沿着time steps求mean和std 归一化后advantages mean=0,std=1
    ...
    def train_critic(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        # pi = actions_prob.view(-1, self.n_actions)
        # Calculated baseline
        baseline = torch.sum(actions_prob * q_targets, dim=-1).view(episode_num, max_episode_len, -1).detach()  # (1,60,3)
