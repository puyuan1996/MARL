...
class QMIX_PG():
    def __init__(self,agent, args):
        ...
        self.model_dir = args.model_dir + '/' + 'ac_local-total'+'/' + args.alg + '/' + args.map  # TODO
        # 如果存在模型则加载模型
    ...
    def train_actor(self, batch, max_episode_len):#EpisodeBatch
        ...
        # Calculated baseline
        q_taken = torch.gather(q_vals, dim=3, index=actions).squeeze(3) #(1,60,3)
        q_taken=q_taken.view(-1)
        # q_taken = self.eval_qmix_net(q_taken, s).detach() # TODO # (1,60,1)
        # q_taken = q_taken.repeat(repeats=(1,1,self.n_agents)).view(-1)  # (1,60,3)->180
        ...
        q_total_target = self.eval_qmix_net(q_targets_mean, s).detach() # TODO # (1,60,1)
        q_total_target = q_total_target.repeat(repeats=(1, 1, self.n_agents)) # (1,60,3)
        baseline = q_total_target.view(-1)
