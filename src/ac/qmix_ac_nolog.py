...
class QMIX_PG():
    def __init__(self, agent, args):
        ...
        self.model_dir = args.model_dir + '/' + 'ac_nolog' +  '/' + f'{args.actor_buffer_size}_'+f'{args.actor_update_delay}_'+f'{args.actor_train_steps}'+ '/' + args.alg + '/' + args.map  # TODO
        # 如果存在模型则加载模型
    ...
    def train_actor(self, batch, max_episode_len):  # EpisodeBatch
        ...
        for transition_idx in range(max_episode_len):
            ...
            agent_outs[reshaped_avail_actions == 0] = -1e11 #todo
            # agent_outs = torch.nn.functional.softmax(agent_outs, dim=-1)
            agent_outs = gumbel_softmax(agent_outs, hard=False)
            # agent_outs = gumbel_softmax(agent_outs, hard=True) # todo new one-hot向量，可导
        ...
        q_mean_actions = torch.sum(actions_prob * q_vals, dim=-1) # (1,60,3)
        # s [1,60,8]
        Q_mean_actions = self.eval_qmix_net(q_mean_actions, s)  # TODO # (1,60,1) # q_i(mean_action), q_-i(mean_action)
        ...
        Q_i_mean_negi_taken = self.eval_qmix_net(q_i_mean_negi_taken, s_repeat)  # TODO #  (1,60*3,1)
        ...
        advantages = Q_i_mean_negi_taken.view(-1)
        # advantages = (q_taken.view(-1) - baseline.view(-1)).detach()
        # advantages = (advantages -log_pi_taken) # TODO

        # policy_loss = - ((advantages * log_pi_taken) * mask).sum() / mask.sum()
        policy_loss = - (advantages * mask).sum() / mask.sum()
    ...
    def train_critic(self, batch, max_episode_len, train_step,
        ...
        for transition_idx in range(max_episode_len):
            ...
            agent_outs = gumbel_softmax(agent_outs, hard=True)  # one-hot  TODO ac_mean
            agent_outs = agent_outs.view(episode_num, self.n_agents, -1)
            actions_prob.append(agent_outs)  # 每个动作的概率
