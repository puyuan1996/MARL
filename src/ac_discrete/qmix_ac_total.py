...
class QMIX_PG():
    def __init__(self,agent, args):
        ...
        tmp = f'tanh_{args.temp}_{args.anneal_epsilon}_' + f'{args.actor_buffer_size}_{args.actor_update_delay}_{args.actor_train_steps}_{args.critic_lr}'
        self.model_dir = args.model_dir + '_my_softmax_evalmax_epsilon'+'/' + 'ac_total' +'/' + tmp+ '/' + args.alg + '/' + args.map  # TODO
        # 如果存在模型则加载模型
    ...
    def train_actor(self, batch, max_episode_len):#EpisodeBatch
        ...
        q_i_mean_negi_mean = torch.sum(actions_prob * q_vals, dim=-1).detach()  # (1,60,3)
        Q_i_mean_negi_mean = self.eval_qmix_net(q_i_mean_negi_mean, s).detach() # TODO # (1,60,1)
    ...
    def train_critic(self, batch, max_episode_len, train_step):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        q_taken_actions = torch.gather(q_evals, dim=3, index=u).squeeze(3)
        ...
        # pi = actions_prob.view(-1, self.n_actions)
        # Calculated baseline
        q_targets_all_mean = torch.sum(actions_prob * q_targets, dim=-1).view(episode_num, max_episode_len, -1).detach()  # (1,60,3)
        ...
        Q_taken_actions = self.eval_qmix_net(q_taken_actions, s) # [1, 60, 1]
        Q_targets_all_mean = self.target_qmix_net(q_targets_all_mean, s_next)

        targets = r + self.args.gamma * Q_targets_all_mean * (1 - terminated)

        td_error = (Q_taken_actions - targets.detach())
