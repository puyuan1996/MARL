...
import numpy as np
torch.cuda.set_device(3)  # id=0, 1, 2 ,4等
...
class QMIX_PG():
    def __init__(self, agent, args):
        ...
        tmp = f'clamp2-5_rewardscale10_' + f'{args.ppo_buffer_size}_{args.actor_train_steps}_{args.critic_lr}'  # f'clamp2-5_'+

        self.model_dir = args.model_dir+'/linear_mix/'+'qmix_ppo' + '/' + tmp + '/' + args.map  # _gradclip0.5
        # 如果存在模型则加载模型
    ...
    def train_actor(self, batch, max_episode_len, epoch):  # EpisodeBatch
        ...
        # actions = batch["u"][:, :-1]
        actions = batch["u"]
        minibatch_oldlogproba = batch["u_logp"]
        ...
        if self.args.cuda:
            ...
            minibatch_oldlogproba = minibatch_oldlogproba.cuda()
        ...
        # samples = torch.multinomial(actions_prob.view(-1, self.n_actions), 1, replacement=True)  # a.shape = (batch_size, num_a)
        # cur_pol_sample_actions=samples.view(episode_num,-1,self.n_agents,1) #max_episode_len

        # Calculated baseline
        q_curpi_sample_actions = torch.gather(q_vals, dim=3, index=actions).squeeze(
            3)  # (1,60,3)  TODO NOTE not cur_pol_sample_actions
        ...
        # pi_sample = torch.gather(pi, dim=1, index=actions.reshape(-1, 1)).squeeze(1)  # cur_pol_sample_actions
        # pi_sample[mask == 0] = 1.0
        # log_pi_sample = torch.log(pi_sample)

        log_pi_taken = torch.gather(log_prob_pi.view(-1, self.n_actions), dim=1, index=actions.reshape(-1, 1)).squeeze(1)

        # advantages = (-self.alpha * log_pi_sample + (Q_curpi_sample.view(-1) - Q_i_mean_negi_sample.view(-1)).detach() ) # TODO
        advantages = (Q_curpi_sample.view(-1) - Q_i_mean_negi_sample.view(-1)).detach()  # TODO
        ...
        if self.args.schedule_clip == 'linear':
            ep_ratio = 1 - (epoch / self.args.n_epoch)
            clip_now = self.args.clip * ep_ratio
        else:
            clip_now = 0.2
        ratio = torch.exp(log_pi_taken - minibatch_oldlogproba)
        surr1 = ratio * advantages * mask
        surr2 = ratio.clamp(1 - clip_now, 1 + clip_now) * advantages * mask
        # loss_surr = - torch.mean(torch.min(surr1, surr2))
        policy_loss = - torch.min(surr1.sum() / mask.sum(), surr2.sum() / mask.sum())
        #
    ...
    def train_critic(self, batch, max_episode_len, train_step,
        ...
        mask = mask.repeat(1, 1, self.n_agents)
        r = 10. * (r - r.mean()) / (
                    r.std() + 1e-6)  # normalize with batch mean and std; plus a small number to prevent numerical problem
        ...
        ##### sac
        # q_targets_sample = torch.sum(actions_prob * (q_targets - self.alpha * log_prob_pi), dim=-1).view(  episode_num, max_episode_len, -1).detach()  # (1,60,3) TODO ac_mean
        # q_targets_sample_2 = torch.sum(actions_prob * (q_targets_2 - self.alpha * log_prob_pi),dim=-1).view(episode_num, max_episode_len, -1).detach()  # (1,60,3)

        ##### ppo 2q
        q_targets_sample = torch.sum(actions_prob * q_targets, dim=-1).view(episode_num, max_episode_len,
                                                                            -1).detach()  # (1,60,3) TODO ac_mean
        q_targets_sample_2 = torch.sum(actions_prob * q_targets_2, dim=-1).view(episode_num, max_episode_len,
                                                                                -1).detach()  # (1,60,3)
