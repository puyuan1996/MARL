...
class QMIX_PG():
    ...
    def train_actor(self, batch, max_episode_len, actor_sample_times=1):  # EpisodeBatch
        ...
        # todo
        samples = torch.multinomial(actions_prob.view(-1, self.n_actions), actor_sample_times,replacement=True)  # a.shape = (batch_size, num_a)
        cur_pol_sample_actions = samples.view(episode_num, -1, self.n_agents, actor_sample_times)  # max_episode_len
        ##### actor_sample_times=1
        # q_curpi_sample_actions = torch.gather(q_vals, dim=3, index=1).squeeze(3)  # (1,60,3)

        ##### actor_sample_times
        q_curpi_sample_actions = torch.gather(q_vals, dim=3,index=cur_pol_sample_actions)  # .squeeze(3)  # (1,60,3)  todo actor_sample_times
        ...
        q_i_mean_negi_sample = q_i_mean_negi_sample.view(episode_num*actor_sample_times, -1, self.n_agents)  # [1, 60*3, 3]
        ...
        # s_repeat = s_repeat.view(episode_num*actor_sample_times, self.n_agents, -1)  # (1,60,48*3)-> # (1,60*3,48)
        s_repeat = s_repeat.view(episode_num * actor_sample_times, self.n_agents*max_episode_len, self.args.state_shape)  # (1,60,48*3)-> # (1,60*3,48)
        ...
        Q_i_mean_negi_sample = Q_i_mean_negi_sample.view(episode_num*actor_sample_times, -1, self.n_agents)  # (1,60*3,1)->(1,60,3)
        ###方法1
        ...
        log_pi_sample = torch.gather(log_prob_pi, dim=3, index=cur_pol_sample_actions)  # todo actor_sample_times
        log_pi_sample = log_pi_sample.permute(3, 0, 1, 2).reshape(-1)


        # advantages = (Q_taken_actions.view(-1) - Q_i_mean_negi_mean.view(-1)).detach()
        advantages = (-self.alpha * log_pi_sample + (
                    Q_curpi_sample.view(-1) - Q_i_mean_negi_sample.view(-1)).detach())  # TODO
