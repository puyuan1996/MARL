...
class QMIX_PG():
    def __init__(self, agent, args):
        ...
        tmp = f'{args.anneal_epsilon}_' + f'{args.actor_buffer_size}_{args.actor_update_delay}_{args.actor_train_steps}_{args.critic_lr}'
        self.model_dir = args.model_dir + '_my_softmax_evalmax_epsilon'+'/' + 'ac_total_counterfactual_1' +  '/' +tmp+ '/' + args.alg + '/' + args.map  # TODO
        # self.model_dir = args.model_dir + '_my_softmax'+'/' + 'ac_total_counterfactual_1' + '/' + f'{args.actor_buffer_size}_'+f'{args.actor_update_delay}_'+f'{args.actor_train_steps}'+ '/' + args.alg + '/' + args.map  # TODO
        # 如果存在模型则加载模型
    ...
    def train_actor(self, batch, max_episode_len):  # EpisodeBatch
        ...
        for transition_idx in range(max_episode_len):
            ...
            # agent_outs, self.policy_hidden = self.target_policy(inputs, self.policy_hidden)
        ...
        # Calculated baseline
        q_taken_actions = torch.gather(q_vals, dim=3, index=actions).squeeze(3)  # (1,60,3)  TODO NOTE # q_i(tacken_action), q_-i(tacken_action)
        ...
        # s [1,60,8]
        Q_mean_actions = self.eval_qmix_net(q_mean_actions, s).detach()  # TODO # (1,60,1) # q_i(mean_action), q_-i(mean_action)
        ...
        # s_repeat = s.repeat(repeats=(1, self.n_agents, 1))  # (1,60,48)-> # (1,60*3,48) #TODO note 顺序错误
        ...
        total_policy_loss.backward()  # policy gradient
        ...
        #     p_sum += p.data.abs().sum().item() # 2107.
        # if p_sum>1e5:
        #     import time
        #     time.sleep(1)
